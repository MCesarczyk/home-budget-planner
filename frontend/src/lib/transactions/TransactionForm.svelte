<script lang="ts">
	...
	import type { Account, Category, Subcategory, Transaction, TransactionType } from './types';
	...
	// Seed field state once from the record being edited (null for a new one).
	function seed() {
		const accountId =
			initial?.type === 'expense'
				? (initial.source_account?.id ?? '')
				: initial?.type === 'income'
					? (initial.destination_account?.id ?? '')
					: '';
		return {
			type: initial?.type ?? 'expense',
			amount: initial ? Number(initial.amount) : null,
			txDate: initial?.tx_date ?? today(),
			comment: initial?.comment ?? '',
			accountId: String(accountId),
			sourceId: initial?.source_account ? String(initial.source_account.id) : '',
			destinationId: initial?.destination_account ? String(initial.destination_account.id) : '',
			categoryId: initial?.subcategory ? String(initial.subcategory.category.id) : '',
			subcategoryId: initial?.subcategory ? String(initial.subcategory.id) : ''
		};
	}
	const seeded = seed();

	let type = $state<TransactionType>(seeded.type);
	let amount = $state<number | null>(seeded.amount);
	let txDate = $state(seeded.txDate);
	let comment = $state(seeded.comment);
	let accountId = $state(seeded.accountId);
	let sourceId = $state(seeded.sourceId);
	let destinationId = $state(seeded.destinationId);
	let categoryId = $state(seeded.categoryId);
	let subcategoryId = $state(seeded.subcategoryId);
	...
	function handleSubmit(e: SubmitEvent) {
		...
		onsubmit({
			...
			source_account: type === 'expense' ? Number(accountId) : null,
			destination_account: type === 'income' ? Number(accountId) : null
		});
	}
</script>
