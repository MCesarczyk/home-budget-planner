...
<main class="flex min-h-screen items-center justify-center bg-slate-50 p-4">
	<div class="w-full max-w-sm rounded-xl bg-white p-8 shadow-sm ring-1 ring-slate-200">
		...
		<p class="mt-1 text-sm text-slate-500">Home Budget Planner</p>
		...
		<form class="mt-6 space-y-4" onsubmit={handleSubmit}>
			...
			<div>
				...
				<input
					...
					bind:value={username}
					disabled={submitting}
					class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-slate-500 focus:ring-slate-500 disabled:opacity-60"
				/>
			</div>
			...
			<button
				type="submit"
				disabled={submitting}
				class="flex w-full justify-center rounded-md bg-slate-900 px-4 py-2 text-sm font-medium text-white hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-500 focus:ring-offset-2 disabled:opacity-60"
			>
