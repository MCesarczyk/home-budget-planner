...
<main class="flex min-h-screen items-center justify-center bg-slate-50 p-4 dark:bg-slate-950">
	...
	{:else if auth.user}
		...
		>
			...
			<p class="mt-1 text-lg font-semibold text-slate-900 dark:text-slate-100">{auth.user.username}</p>
			<button
				type="button"
				onclick={handleLogout}
				class="mt-6 w-full rounded-md bg-slate-900 px-4 py-2 text-sm font-medium text-white hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-500 focus:ring-offset-2 dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-slate-200 dark:focus:ring-slate-400 dark:focus:ring-offset-slate-900"
			>
				Sign out
			</button>
		</div>
